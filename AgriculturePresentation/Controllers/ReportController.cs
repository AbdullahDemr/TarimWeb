...
namespace AgriculturePresentation.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public IActionResult AnnouncementReport()
        {
            using (var workBook = new XLWorkbook())
            {
                ...
                foreach (var item in AnnouncementList())
                {
                    ...
                    workSheet.Cell(contactRowCount, 2).Value = item.Imageurl;
                    workSheet.Cell(contactRowCount, 3).Value = item.Title;
                    workSheet.Cell(contactRowCount, 4).Value = item.Date;
                    workSheet.Cell(contactRowCount, 5).Value = item.Description;
                    workSheet.Cell(contactRowCount, 6).Value = item.MinDescription;
                    workSheet.Cell(contactRowCount, 7).Value = item.Status;
                    contactRowCount++;
                }
        ...
        public List<TopAnnouncementModel> TopAnnouncementList()
        {
            List<TopAnnouncementModel> topAnnouncementModels = new List<TopAnnouncementModel>();
            using (var context = new AgricultureContext())
            {
                topAnnouncementModels = context.TopAnnouncements.Select(x => new TopAnnouncementModel
                {

                    TopAnnouncementId = x.TopAnnouncementId,
                    Status = x.Status,
                    MinDescription = x.MinDescription,
                    Imageurl = x.Imageurl,
                    Date = x.Date,
            ...
            return topAnnouncementModels;
        }

        public IActionResult TopAnnouncementReport()
        {
            using (var workBook = new XLWorkbook())
            {
                var workSheet = workBook.Worksheets.Add("Ust Duyuru Listesi");
                workSheet.Cell(1, 1).Value = "Ust Duyuru ID";
                workSheet.Cell(1, 1).Value = "Ust Duyuru Gorsel";
                workSheet.Cell(1, 2).Value = "Ust Duyuru Başlığı";
                workSheet.Cell(1, 3).Value = "Ust Duyuru Tarihi";
                workSheet.Cell(1, 4).Value = "Ust Duyuru İçeriği";
                workSheet.Cell(1, 4).Value = "Ust Duyuru Kısa İçeriği";
                workSheet.Cell(1, 5).Value = "Ust Durum";

                int contactRowCount = 2;
                foreach (var item in TopAnnouncementList())
                {
                    workSheet.Cell(contactRowCount, 1).Value = item.TopAnnouncementId;
                    workSheet.Cell(contactRowCount, 2).Value = item.Imageurl;
                    workSheet.Cell(contactRowCount, 3).Value = item.Title;
                    workSheet.Cell(contactRowCount, 4).Value = item.Date;
                    workSheet.Cell(contactRowCount, 5).Value = item.Description;
